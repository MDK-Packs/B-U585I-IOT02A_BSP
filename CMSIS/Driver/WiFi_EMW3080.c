 ...
 * Documentation:       ./Doc/WiFi_EMW3080_README.md
 * --------------------------------------------------------------------------
...
// Import STM32Cube Mx WiFi Driver function
extern void MX_WIFI_NOTIFY_ISR          (void);
extern void MX_WIFI_FLOW_ISR            (void);
...
static int32_t WiFi_SocketCreate (int32_t af, int32_t type, int32_t protocol) {
  ...
  switch (protocol) {
    case ARM_SOCKET_IPPROTO_TCP:
      mx_protocol = IPPROTO_TCP;
      break;
    case ARM_SOCKET_IPPROTO_UDP:
      mx_protocol = IPPROTO_UDP;
      break;
    default:
      return ARM_SOCKET_EINVAL;
  }
  ...
  if (osMutexAcquire(mutex_id_sock_attr, WIFI_EMW3080_SOCKETS_TIMEOUT) == osOK) {
    ...
    if ((rc >= 0) && (rc < WIFI_EMW3080_SOCKETS_NUM)) {         // If create has succeeded and socket number is valid
      ...
      val = 1;
      (void)MX_WIFI_Socket_setsockopt(ptrMX_WIFIObject, rc, SOL_SOCKET, (int32_t)SO_RCVTIMEO, &val, 4);
...
static int32_t WiFi_SocketBind (int32_t socket, const uint8_t *ip, uint32_t ip_len, uint16_t port) {
  ...
  if (osMutexAcquire(mutex_id_sock_attr, WIFI_EMW3080_SOCKETS_TIMEOUT) == osOK) {
    ...
    if (sock_attr[socket].flags.created == 0U) {
      ...
      if (rc == 0) {
        rc = MX_WIFI_Socket_bind(ptrMX_WIFIObject, socket, (const struct sockaddr *)&addr, addr_len);
...
static int32_t WiFi_SocketAccept (int32_t socket, uint8_t *ip, uint32_t *ip_len, uint16_t *port) {
  ...
  if (rc == 0) {
    ...
    do {
      if (osMutexAcquire(mutex_id_sock_attr, WIFI_EMW3080_SOCKETS_TIMEOUT) == osOK) {
        rc = MX_WIFI_Socket_accept(ptrMX_WIFIObject, socket, (struct sockaddr *)&addr, (uint32_t *)&addr_len);
        if ((rc >= 0) && (rc < WIFI_EMW3080_SOCKETS_NUM)) {       // If accept has succeeded and socket number is valid
          ...
          if (addr.ss_family == (uint8_t)AF_INET) {
...
static int32_t WiFi_SocketConnect (int32_t socket, const uint8_t *ip, uint32_t ip_len, uint16_t port) {
  ...
  if (osMutexAcquire(mutex_id_sock_attr, WIFI_EMW3080_SOCKETS_TIMEOUT) == osOK) {
    ...
    if (sock_attr[socket].flags.created == 0U) {
      ...
      rc = MX_WIFI_Socket_connect(ptrMX_WIFIObject, socket, (struct sockaddr *)&addr, addr_len);
    ...
    do {
      if (osMutexAcquire(mutex_id_sock_attr, WIFI_EMW3080_SOCKETS_TIMEOUT) == osOK) {
        if (len == 0U) {                        // if len = 0, try to receive to local buffer
          rc = MX_WIFI_Socket_recvfrom(ptrMX_WIFIObject, socket, (uint8_t *)sock_attr[socket].rx_buf, WIFI_EMW3080_SOCKETS_RX_BUF_SIZE, 0, (struct sockaddr *)&addr, (uint32_t *)&addr_len);
          if (rc > 0) {                         // If something was received
            // Store remote IP address and port, data is received int local buffer
            sock_attr[socket].rx_buf_available_len = (uint16_t)rc;
            const SOCKADDR_IN *sa = (SOCKADDR_IN *)&addr;
            if ((sa->sin_family == (uint8_t)AF_INET) && (sizeof(sa->sin_addr) >= 4U)) {
          ...
          rc = MX_WIFI_Socket_recvfrom(ptrMX_WIFIObject, socket, (uint8_t *)buf, (int32_t)len, 0, (struct sockaddr *)&addr, (uint32_t *)&addr_len);
          if (rc > 0) {                         // If something was received
            // Store remote IP address and port, data is already in buffer
            if ((ip != NULL) && (ip_len != NULL)) {
              const SOCKADDR_IN *sa = (SOCKADDR_IN *)&addr;
              if ((sa->sin_family == (uint8_t)AF_INET) && (*ip_len >= sizeof(sa->sin_addr))) {
                memcpy(ip, &sa->sin_addr, sizeof(sa->sin_addr));
                *ip_len = sizeof(sa->sin_addr);
              }
              if (port != NULL) {
...
static int32_t WiFi_SocketSendTo (int32_t socket, const void *buf, uint32_t len, const uint8_t *ip, uint32_t ip_len, uint16_t port) {
  ...
  if (osMutexAcquire(mutex_id_sock_attr, WIFI_EMW3080_SOCKETS_TIMEOUT) == osOK) {
    ...
    if (sock_attr[socket].flags.created == 0U) {
      ...
      for (retry = 3U; retry != 0U; retry--) {
        rc = MX_WIFI_Socket_sendto(ptrMX_WIFIObject, socket, (uint8_t *)buf, (int32_t)len, 0, (struct sockaddr *)ptr_addr, addr_len);
        if (rc > 0) {
          break;
        }
        (void)osDelay(10U);
...
static int32_t WiFi_SocketGetSockName (int32_t socket, uint8_t *ip, uint32_t *ip_len, uint16_t *port) {
  ...
  if (osMutexAcquire(mutex_id_sock_attr, WIFI_EMW3080_SOCKETS_TIMEOUT) == osOK) {
    ...
    if (sock_attr[socket].flags.created == 0U) {
      ...
      rc = MX_WIFI_Socket_getsockname(ptrMX_WIFIObject, socket, (struct sockaddr *)&addr, (uint32_t *)&addr_len);
...
static int32_t WiFi_SocketGetPeerName (int32_t socket, uint8_t *ip, uint32_t *ip_len, uint16_t *port) {
  ...
  if (osMutexAcquire(mutex_id_sock_attr, WIFI_EMW3080_SOCKETS_TIMEOUT) == osOK) {
    ...
    if (sock_attr[socket].flags.created == 0U) {
      ...
      rc = MX_WIFI_Socket_getpeername(ptrMX_WIFIObject, socket, (struct sockaddr *)&addr, (uint32_t *)&addr_len);
...
//        rc = MX_WIFI_Socket_getsockopt(ptrMX_WIFIObject, socket, SOL_SOCKET, (int32_t)SO_RCVTIMEO,  opt_val, &len);
...
//        MX_WIFI_Socket_getsockopt function returns error, so value is taken from sock_attr
//        rc = MX_WIFI_Socket_getsockopt(ptrMX_WIFIObject, socket, SOL_SOCKET, (int32_t)SO_SNDTIMEO,  opt_val, &len);
          *((uint32_t *)opt_val) = sock_attr[socket].sndtimeo;
          *opt_len = 4U;
          rc = 0;
          break;
        case ARM_SOCKET_SO_KEEPALIVE:
          rc = MX_WIFI_Socket_getsockopt(ptrMX_WIFIObject, socket, SOL_SOCKET, (int32_t)SO_KEEPALIVE, opt_val, &len);
          if (rc == 0) {
            *opt_len = len;
          } else if (rc < 0) {
            rc = ConvertSocketErrorCodeMxToCmsis(rc);
        ...
        case ARM_SOCKET_SO_TYPE:
          rc = MX_WIFI_Socket_getsockopt(ptrMX_WIFIObject, socket, SOL_SOCKET, (int32_t)SO_TYPE,      opt_val, &len);
          if (rc == 0) {
            *opt_len = len;
          } else if (rc < 0) {
            rc = ConvertSocketErrorCodeMxToCmsis(rc);
    ...
    if (sock_attr[socket].flags.created == 0U) {
      ...
      switch (opt_id) {
        ...
        case ARM_SOCKET_SO_RCVTIMEO:
          ...
          break;
        case ARM_SOCKET_SO_SNDTIMEO:
          rc = MX_WIFI_Socket_setsockopt(ptrMX_WIFIObject, socket, SOL_SOCKET, (int32_t)SO_SNDTIMEO,  opt_val, 4);
          if (rc == 0) {
            sock_attr[socket].sndtimeo = *((uint32_t *)opt_val);
          }
          break;
        case ARM_SOCKET_SO_KEEPALIVE:
          rc = MX_WIFI_Socket_setsockopt(ptrMX_WIFIObject, socket, SOL_SOCKET, (int32_t)SO_KEEPALIVE, opt_val, 4);
          break;
        case ARM_SOCKET_SO_TYPE:
          rc = ARM_SOCKET_EINVAL;
          break;
...
static int32_t WiFi_SocketGetHostByName (const char *name, int32_t af, uint8_t *ip, uint32_t *ip_len) {
  ...
  // Resolve hostname
  rc = MX_WIFI_Socket_gethostbyname(ptrMX_WIFIObject, (struct sockaddr *)&addr, (char *)name);
  if (rc < 0) {
