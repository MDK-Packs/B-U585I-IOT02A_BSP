...
#else
#define LOCK_DECLARE(A)                           osMutexId_t A
#define LOCK_INIT(A)                              A = osMutexNew(NULL)
#define LOCK(A)                                   if (osMutexAcquire((A), osWaitForever) != osOK) {while(1){};}
#endif /* defined(osCMSIS) && (osCMSIS < 0x20000U) */
...
#else
#define SEM_DECLARE(A)                            osSemaphoreId_t A
#define SEMAPHORE_DEF(NAME)                       
#define SEMAPHORE(NAME)                           NULL
#endif /* defined(osCMSIS) && (osCMSIS < 0x20000U) */
...
#else
#define THREAD_DECLARE(A)                         osThreadId_t A
...
#else
#define FIFO_DECLARE(QUEUE)                       osMessageQueueId_t QUEUE
